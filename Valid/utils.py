...
NP_model = pickle.load(gzip.open('pre_model/NP_score.pkl.gz'))
SA_model = {i[j]: float(i[0]) for i in pickle.load(gzip.open('pre_model/SA_score.pkl.gz')) for j in range(1, len(i))}
...
def all_scores(mols, data, smiles, norm=True, reconstruction=False):
    ...
    m1 = {'valid score': MolecularMetrics.valid_total_score(mols) * 100,
          ...
          'novel score': MolecularMetrics.novel_total_score(mols, smiles) * 100}
